...
public class CheckPriorityRoom {
	...
	public int priorityMalfunzionamenti(Stanza stanza) {
			...
			if (cont == 1) {prioritymalf=1;} //se c'è solo un malfunzionamento
			
			if (cont > 1) {prioritymalf=2;} //se c'è solo un malfunzionamento
					
			if (stanza.typeWorking().size() < 4 && stanza.typeWorking().size()>0)
				{prioritymalf=3;} //c'è almeno un tipo di sensore che non rileva
